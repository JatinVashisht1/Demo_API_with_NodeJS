...
function verfyApiKey(tokenParts) {
    // const tokenParts = req.params.apikey;
    console.log(tokenParts)
    if (tokenParts.match(/\S+\.\S+\.\S+/) !== null) {
        try {
            const verification = jsonwebtoken.verify(tokenParts, PUB_KEY, {
                algorithms: ['RS256']
            })
            console.log('verification is ', verification)
            // req.jwt = verification;
...
const middleware = (req, res, next) => {
    ...
    console.log(apikey);
    if (apikey.match(/\S+\.\S+\.\S+/) !== null) {
        try {
            const verification = jsonwebtoken.verify(apikey, PUB_KEY, {
                algorithms: ['RS256']
            })
            console.log('verification is ', verification)
            // req.jwt = verification;
...
module.exports.verfyApiKey = verfyApiKey;
module.exports.issueApiKey = issueApiKey;
